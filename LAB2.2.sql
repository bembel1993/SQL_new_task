...
--2) ВЫБОРКА СОТРУДНИКОВ ПРОХОДИВШИХ ОБУЧЕНИЕ В КАЛИФОРНИИ
SELECT 
	HRD.NAMEFL,
	DE.NAMEUNIVERSITY,
	DE.LOCATIONUNIVERSITY,
...
--3) Найти сотрудников с одинаковым заработком.
SELECT 
	HRD.NAMEFL,
	DD.EARNINGS,
	DD.IDEMPLOYEE
...
SELECT 
	...
	EARNINGS_AVG = (SELECT AVG(EARNINGS)
					FROM DETAILS_DEPARTMENT)
FROM DETAILS_DEPARTMENT 
WHERE EARNINGS < (
...
--8) ВЫБРАТЬ СОТРУДНИКОВ С ОДИНАКОВОЙ ЗАРАБОТНОЙ ПЛАТОЙ.
SELECT 
	FIRST.EARNINGS,
